...
test("stepper", async (t) => {
  const exampleSlideBuilds = [
    // Slide 0: single step
    [],
    // Slide 1: two steps (numbered from -1)
    [
      { stepNumbers: [-1, 0], tags: [] },
      { stepNumbers: [0], tags: [] },
    ],
    // Slide 2: three steps (numbered from 0), with tags
    [{ stepNumbers: [0, 1, 2] }, { stepNumbers: [1, 2] }, { stepNumbers: [2] }],
  ];
  ...
  await t.test("initial defaults to step 0, slide 0", async (t) => {
    assert.deepEqual(new Stepper(exampleSlideBuilds).state, {
      slide: 0,
      step: 0,
      stepNumber: 0,
      blanked: false,
  ...
  await t.test("initial slide/step can be changed", async (t) => {
    assert.deepEqual(new Stepper(exampleSlideBuilds, 2, 1).state, {
      slide: 2,
      step: 1,
      stepNumber: 1,
      blanked: false,
  ...
  await t.test("initial slide/step out of range", async (t) => {
    assert.deepEqual(new Stepper(exampleSlideBuilds, 1, 99).state, {
      slide: 0,
      step: 0,
      stepNumber: 0,
      blanked: false,
  ...
  await t.test("initial slide out of range", async (t) => {
    assert.deepEqual(new Stepper(exampleSlideBuilds, 9, 99).state, {
      slide: 0,
      step: 0,
      stepNumber: 0,
      blanked: false,
  ...
  await t.test("onChange callback", async (t) => {
    const stepper = new Stepper(exampleSlideBuilds);
    ...
    assert.deepEqual(onChange.mock.calls[0].arguments, [
      { slide: 1, step: 0, stepNumber: -1, blanked: false },
      { slide: 0, step: 0, stepNumber: 0, blanked: false },
    ]);
    ...
    assert.deepEqual(onChange.mock.calls[1].arguments, [
      { slide: 1, step: 0, stepNumber: -1, blanked: true },
      { slide: 1, step: 0, stepNumber: -1, blanked: false },
    ]);
    ...
    assert.deepEqual(onChange.mock.calls[2].arguments, [
      { slide: 2, step: 1, stepNumber: 1, blanked: false },
      { slide: 1, step: 0, stepNumber: -1, blanked: true },
    ]);
    ...
    assert.deepEqual(onChange.mock.calls[4].arguments, [
      { slide: 2, step: 1, stepNumber: 1, blanked: false },
      { slide: 2, step: 1, stepNumber: 1, blanked: true },
    ]);
  });
