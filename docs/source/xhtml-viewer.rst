...
1. **The current slide and step numbers.** These are displayed (and can
   be entered) using the same :ref:`syntax used for hyperlinks <links>`,
   without the leading ``#``. The current position in the presentation is also
   reflected in the browser URL bar.
...
5. **Presenter view button.** Open the :ref:`presenter view <presenter-view>`
   in a new window.

6. **Help button.** Enumerates all keyboard shortcuts supported by the viewer.

7. **Slide thumbnail browser.**
...
.. tip::
    ...
    Hover over the slide number in the thumbnail browser to show the source
    SVG file of that slide.
