...
def test_enumerate_inkscape_layers() -> None:
    ...
    assert len(layers[1].children) == 2
    assert get_inkscape_layer_name(layers[1].children[0].element) == "Middle layer, top sublayer"
    assert get_inkscape_layer_name(layers[1].children[1].element) == "Middle layer, bottom sublayer"
